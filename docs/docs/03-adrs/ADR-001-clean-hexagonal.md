...
## サンプル例: 単一 bounded context の維持

:::note サンプル例
本セクションはタスク管理サンプルでの判断経緯。適用先プロジェクトで業務サブドメインの構成が異なる場合は本セクションごと書き換える（→ [サンプル削除ガイド](../getting-started/strip-sample)）。
:::

タスク管理サンプルは業務サブドメインが「タスク」1 つのみであり、追加の業務サブドメインを抱える計画もないため、中間の業務サブドメイン階層（例: `jp.mufg.it.rcb.task`）は YAGNI と判断し `domain` / `application` / `adapter` をルートパッケージ（`jp.mufg.it.rcb`）直下に配置する構成を採る。
