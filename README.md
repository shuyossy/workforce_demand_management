# sak-dev-env (jakartaEE branch)

Jakarta EE 系列の AI 主導開発に対応したモダンな開発環境ボイラープレート。WildFly 32.0.1.Final + PostgreSQL + Flyway + PrimeFaces によるサンプルアプリ（休暇申請ワークフロー）を内蔵し、適用先プロジェクトはサンプル層を差し替えるだけで、品質ゲート・CI/CD・ドキュメントサイト・Docker ビルドをそのまま利用できる。
...
`src/` 配下に Jakarta EE 10 / WildFly 上で動作する休暇申請ワークフローのサンプルが含まれる。これは使い方を示すための同梱物であり、適用先プロジェクトでは [サンプル削除ガイド](docs/docs/01-getting-started/strip-sample.md) に従って撤去してから自案件のドメインで開発を始める。
...
- 認証：社内認証サーバ越し（junction-path 規約）の運用を前提とした URL ビルダ・セッション保持
...
- `03-adrs/` — Architecture Decision Records（5 件）
...
│   ├── domain/, application/, adapter/  サンプルアプリ本体（休暇申請、適用先で削除→ docs/01-getting-started/strip-sample.md）
│   ├── shared/              ボイラープレート共通基盤（config / security / web / message）
