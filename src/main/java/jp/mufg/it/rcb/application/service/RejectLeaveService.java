...
/** 休暇却下ユースケースの実装（BR-PRE-001/002/004/005, BR-POL-001）. */
@ApplicationScoped
public class RejectLeaveService implements RejectLeaveUseCase {
  ...
  @Override
  @Transactional
  public void reject(@Valid final RejectLeaveCommand cmd) {
    ...
    req.reject(user.getEmpNum(), user.getName(), clock.now(), cmd.getComment());
    repository.save(req);
    // PMD GuardLogStatement: Object[] パラメータ配列生成を回避するため Level ガードを挟む.
    if (sysLogger.isLoggable(Level.INFO)) {
      sysLogger.log(Level.INFO, "RCB00003-I", new Object[] {req.getId(), req.getApplicantName()});
    }
  }
